...
class TCSwitch {
  ...
  public:
    ...
    // 发送原始时序数据（用于无法解码的信号）
    void sendRaw(unsigned int* timings, unsigned int changeCount, unsigned int repeatCount = 1);
    ...
    unsigned int getReceivedChangeCount();  // 获取原始时序数据的变化次数
    #endif
  ...
  private:
    ...
    static unsigned int changeCount;  // 记录当前接收到的变化次数
    #endif
