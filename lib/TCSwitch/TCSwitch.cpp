...
unsigned int TCSwitch::changeCount = 0;
#endif
...
/**
 * 发送原始时序数据（用于无法解码的信号）
 * @param timings 时序数据数组（微秒）
 * @param changeCount 时序数据的变化次数
 * @param repeatCount 重复发送次数
 */
void TCSwitch::sendRaw(unsigned int* timings, unsigned int changeCount, unsigned int repeatCount) {
  if (this->nTransmitterPin == -1 || timings == nullptr || changeCount == 0) {
    return;
  }
  ...
  // 发送指定次数
  for (unsigned int nRepeat = 0; nRepeat < repeatCount; nRepeat++) {
    // 从 HIGH 开始（大多数协议）
    bool level = HIGH;
    ...
    // 如果还有下一次重复，添加一个间隔（使用最后一个时序值作为间隔）
    if (nRepeat < repeatCount - 1 && changeCount > 0) {
      delayMicroseconds(timings[changeCount - 1]);
    }
  }

  // Disable transmit after sending
  digitalWrite(this->nTransmitterPin, LOW);
...
unsigned int TCSwitch::getReceivedChangeCount() {
  return TCSwitch::changeCount;
}
...
void RECEIVE_ATTR TCSwitch::handleInterrupt() {
  ...
  if (duration > TCSwitch::nSeparationLimit) {
    ...
    if ((repeatCount==0) || (diff(duration, TCSwitch::timings[0]) < 200)) {
      ...
      repeatCount++;
      if (repeatCount == 2) {
        TCSwitch::changeCount = changeCount;  // 保存变化次数
